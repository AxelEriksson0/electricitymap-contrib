...
function catchError(e) {
    ...
    thirdPartyServices.ga('event', 'exception', {
        description: e,
        fatal: false
    });
            ...
            exchangeLayer = new ExchangeLayer('arrows-layer', zoneMap)
                ...
                .onExchangeMouseOut((d) => {
                    if (d.co2intensity && co2Colorbars) {
                        co2Colorbars.forEach((c) => {
                            c.currentMarker(undefined);
                        });
                    }
...
countryTable
    ...
    .onExchangeMouseOut(() => {
        if (co2Colorbars) co2Colorbars.forEach((d) => {
            d.currentMarker(undefined);
        });
...
function toggleSolar() {
    if (typeof solarLayer === 'undefined') {
        return;
    }
        ...
        .onCountryClick((d) => {
            ...
            if (getState().application.selectedZoneName !== d.countryCode) {
                dispatch({
                    type: 'UPDATE_SELECTED_ZONE',
                    payload: {
                        selectedZoneName: d.countryCode
                    }
                });
            }
...
function routeToPage(pageName, state) {
    ...
    if (pageName === 'map') {
        ...
        renderMap(state);
        if (state.application.windEnabled && typeof windLayer !== 'undefined') {
            windLayer.show();
        }
        if (state.application.solarEnabled && typeof solarLayer !== 'undefined') {
            solarLayer.show();
        }
        if (co2Colorbars) co2Colorbars.forEach((d) => {
            d.render();
        ...
        if (pageName === 'info') {
            ...
            if (state.application.windEnabled)
                if (windColorbar) windColorbar.render();
            if (state.application.solarEnabled)
                if (solarColorbar) solarColorbar.render();
        }
    }
    ...
    if (typeof zoneMap !== 'undefined') {
        zoneMap.setData(electricityMixMode === 'consumption' ?
            ...
            .map(d => Object.assign({}, d, {
                co2intensity: d.co2intensityProduction
            })));
    }
